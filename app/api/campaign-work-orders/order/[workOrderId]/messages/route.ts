...
export async function GET(request: Request, { params }: Params) {
  ...
  const response = await fetchWithTimeout(`${getCampaignApiBase()}/api/v1/work-orders/${workOrderId}/messages${url.search}`, {
    method: "GET",
    headers: {
...
export async function POST(request: Request, { params }: Params) {
  ...
  const response = await fetchWithTimeout(`${getCampaignApiBase()}/api/v1/work-orders/${workOrderId}/messages`, {
    method: "POST",
    headers: {
      "x-internal-api-key": internalApiKey,
      "x-actor-id": actor.actorId,
