...
export async function GET(request: Request, { params }: Params) {
  ...
  const upstream = `${getCampaignApiBase()}/api/v1/campaigns/${campaignId}/work-orders${url.search}`;
...
export async function POST(request: Request, { params }: Params) {
  ...
  const response = await fetchWithTimeout(`${getCampaignApiBase()}/api/v1/campaigns/${campaignId}/work-orders`, {
    method: "POST",
    headers: {
      "x-internal-api-key": internalApiKey,
      "x-actor-id": actor.actorId,
    ...
    body: JSON.stringify({ ...body, created_by: actor.actorId }),
    cache: "no-store",
  });
  const text = await response.text();
  return new Response(text, {
