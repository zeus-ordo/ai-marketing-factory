...
type ContextRow = { campaign_id: string; generation_context_id: string; run_id: string; created_at: string; activity_type?: string | null; status?: string | null; context_item_count: number; payload_count: number };
...
export default function HomePage() {
  ...
  function clearFilters() {
    setCampaign(""); setFrom(""); setTo(""); setActivityType(""); setStatus(""); setContext(""); setRun("");
  ...
  return <main className="shell">
    ...
    <section className="panel filters" aria-labelledby="filter-heading">
      ...
      <div className="filter-grid">
        <label className="field wide">Campaign<input list="campaign-options" placeholder="Search campaigns" value={campaign} onChange={event => setCampaign(event.target.value)} /><datalist id="campaign-options">{campaigns.map(item => <option key={item.campaign_id} value={item.campaign_id}>{humanize(item.status)}</option>)}</datalist></label>
    ...
    <div className="content-grid">
      <section className="panel activity-panel" aria-labelledby="activity-list-heading"><div className="section-heading"><div><h2 id="activity-list-heading">Recent activity</h2><p>{loading ? "Loading activity..." : "Select an activity to see its context."}</p></div></div>
        {loading ? <div className="empty-state"><span className="spinner" aria-hidden="true" /><p>Loading AI activity...</p></div> : rows.length === 0 ? <div className="empty-state"><div className="empty-icon" aria-hidden="true">--</div><h3>No AI activity found</h3><p>Try clearing a filter or choosing a different date range.</p></div> : <div className="activity-list">{rows.map(row => <article className={`activity-card ${selected?.generation_context_id === row.generation_context_id ? "selected" : ""}`} key={row.generation_context_id}><div className="card-main"><div className="card-title"><span className="activity-dot" aria-hidden="true" /><h3>{humanize(row.activity_type)}</h3><span className={`status status-${row.status ?? "unknown"}`}>{humanize(row.status)}</span></div><p className="campaign-name">{row.campaign_id}</p><p className="timestamp">{formatDate(row.created_at)}</p></div><dl className="card-stats"><div><dt>References</dt><dd>{row.context_item_count}</dd></div><div><dt>Outputs</dt><dd>{row.payload_count}</dd></div></dl><button className="button secondary open-button" onClick={() => selectContext(row.generation_context_id)}>View details<span aria-hidden="true"> &rarr;</span></button></article>)}</div>}
      </section>
