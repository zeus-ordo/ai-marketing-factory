...
function contextItemSummary(item: unknown, index: number) {
  ...
  return { label: value("label", "file_name", "name", "source_id") || `Reference ${index + 1}`, folder: value("folder", "folder_name") || "Folder not recorded", sourceType: humanize(value("source_type") || "Unknown source"), text: value("text", "query", "summary", "description") || "No text or query recorded" };
}
...
function ViewerPage() {
  ...
  const resultText = outputs.length ? jsonText(outputs) : `No generated output was persisted for this activity.\nPersisted outcome: ${humanize(selectedRow?.status ?? "unknown")}.`;
...
function SourcePanel({ selected, workerContext, assembledContext, onCopy, onDownload }: { selected: Detail; workerContext: string; assembledContext: string; onCopy: (value: string, label: string) => void; onDownload: (name: string, value: string, type: string, label: string) => void }) { return <section className="detail-card source-card"><div className="detail-card-heading"><div><h3>Information given to the AI</h3><p>Sources and context included with the instruction. {selected.items.length} source{selected.items.length === 1 ? "" : "s"} recorded.</p></div><div className="detail-actions"><button onClick={() => onCopy(workerContext, "information")}>Copy information</button><button onClick={() => onDownload(`${selected.generation_context_id}-worker-context.txt`, workerContext, "text/plain", "information")}>Download text</button><button onClick={() => onDownload(`${selected.generation_context_id}-worker-context.json`, workerContext, "application/json", "information JSON")}>Download JSON</button></div></div><ul className="source-list">{selected.items.map((item, index) => { const summary = contextItemSummary(item, index); return <li key={index}><strong>Label/file name: {summary.label}</strong><span>Folder: {summary.folder} &middot; Source type: {summary.sourceType}</span><span>{summary.text}</span></li>; })}</ul><details><summary>View all information</summary><pre>{assembledContext}</pre></details></section>; }
