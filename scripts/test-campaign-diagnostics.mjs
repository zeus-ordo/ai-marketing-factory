...
assert.match(campaigns, /t\("status\./);
assert.match(reviewPage, /t\("status\./);
assert.match(provenance, /review\.diagnostics\.sourceTypes/);
assert.doesNotMatch(campaigns, /<span className="text-slate-500">\{task\.status\}<\/span>/);
assert.doesNotMatch(reviewPage, /\{task\.task_type\}: /);
assert.doesNotMatch(provenance, /provenanceText\(source\)/);
...
assert.match(campaigns, /review\.diagnostics\.tokens/);
assert.match(reviewPage, /review\.diagnostics\.tokens/);
assert.match(review, /review\.diagnostics\.sources/);
assert.doesNotMatch(campaigns, /task\.provider \|\| "—"|task\.model \|\| "—"/);
assert.doesNotMatch(review, /task\.provider \|\| "—"|task\.model \|\| "—"|return "—"/);
